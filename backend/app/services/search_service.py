...
from app.services.suppliers.mock_flight_client import AIRPORTS, mock_flight_client
from app.services.suppliers.mock_hotel_client import CITIES, mock_hotel_client
...
class SearchService:
    ...
    async def search_flights(
        db: AsyncSession, request: FlightSearchRequest, current_user: Employee
    ) -> FlightSearchResponse:
        """
        ...
        raw_offers = mock_flight_client.search_flights(
            ...
            return_date=datetime.combine(request.return_date, datetime.min.time())
    ...
    async def search_hotels(
        db: AsyncSession, request: HotelSearchRequest, current_user: Employee
    ) -> HotelSearchResponse:
        """
        ...
        checkout_dt = (
            datetime.combine(request.checkout_date, datetime.min.time())
        ...
        for raw in raw_offers:
            # Apply policy tagging
            policy_status, policy_notes = SearchService._tag_hotel_policy(raw, policy_settings)
    ...
    def _sort_flight_offers(offers: list[FlightOffer], sort_by: SortBy) -> list[FlightOffer]:
        ...
        if sort_by == SortBy.DEPARTURE:
            return sorted(
                offers, key=lambda x: x.segments[0].departure_time if x.segments else datetime.max
            )
        if sort_by == SortBy.ARRIVAL:
            return sorted(
                offers, key=lambda x: x.segments[-1].arrival_time if x.segments else datetime.max
            )
        return offers
    ...
    def _tag_flight_policy(
        offer: dict, policy_settings: dict, user: Employee
        ...
        if cabin_class in {"business", "first"}:
            allowed_titles = policy_settings.get(
    ...
    @staticmethod
    def _tag_hotel_policy(offer: dict, policy_settings: dict) -> tuple[PolicyStatus, list[str]]:
        ...
        if price_per_night > max_hotel_rate * 1.5:
            status = PolicyStatus.VIOLATION
            notes.append(
                f"Rate ${price_per_night:.0f}/night exceeds limit ${max_hotel_rate:.0f} by more than 50%"
            )
