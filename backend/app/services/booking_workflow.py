...
class BookingStateMachine:
    ...
    async def submit_draft(db: AsyncSession, booking: Booking, policy_result: dict) -> Booking:
        ...
        if approval_req:
            ...
            if not manager:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="Approval required but no active manager found in hierarchy.",
                )

            # 2. Create Approval Request
            req = ApprovalRequest(
                booking_id=booking.id, approver_id=manager.id, status=ApprovalStatus.PENDING
            )
            db.add(req)
    ...
    async def approve_booking(db: AsyncSession, booking: Booking, approver: Employee) -> Booking:
        ...
        AuditService.log(
            ...
            approver.id,
            "APPROVE",
            from_state=str(old_status.value),
            to_state=str(BookingStatus.APPROVED.value),
        )
    ...
    async def reject_booking(db: AsyncSession, booking: Booking, approver: Employee) -> Booking:
        ...
        AuditService.log(
            ...
            "REJECT",
            from_state=str(old_status.value),
            to_state=str(BookingStatus.REJECTED.value),
        )
