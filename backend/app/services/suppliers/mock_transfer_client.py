...
CANCEL_REASONS = [
    {
        "id": 1,
        "cancellation_name": "Flight cancelled",
        "cancellation_description": "My flight was cancelled",
    },
    {
        "id": 2,
        "cancellation_name": "Flight delayed",
        "cancellation_description": "Significant flight delay",
    },
    {
        "id": 3,
        "cancellation_name": "Change of plans",
        "cancellation_description": "Travel plans changed",
    },
    {
        "id": 4,
        "cancellation_name": "Found alternative",
        "cancellation_description": "Found another transportation option",
    },
    {
        "id": 5,
        "cancellation_name": "Booking error",
        "cancellation_description": "Made a booking mistake",
    },
...
class MockTransferClient:
    ...
    async def get_quotes(
        ...
        return TransferQuoteResponse(
            ...
            airport=QuoteAirport(
                id=airport_info["id"], name=airport_info["name"], code=airport_info["code"]
            ),
            vehicles=vehicles,
            distance=distance,
    ...
    async def get_booking(self, reservation_no: str) -> TransferBookingDetails:
        ...
        if booking["status"] == TransferStatus.APPROVED.value:
            driver = DriverInfo(
                name="James Smith", phone="+44 7700 900123", vehicle_plate="AB12 CDE"
            )
    ...
    async def cancel_booking(
        ...
        return TransferCancelResponse(
            status="Success", message="Refund successful.", refund_amount=refund
        )
    ...
    def _find_airport(self, pickup: Location, dropoff: Location) -> dict:
        ...
        # Check pickup first
        for airport in AIRPORTS:
            if pickup.type == LocationType.AIRPORT and (
                str(pickup.location_id) == str(airport["id"])
                or str(pickup.location_id).upper() == airport["code"]
            ):
                return airport

        # Check dropoff
        for airport in AIRPORTS:
            if dropoff.type == LocationType.AIRPORT and (
                str(dropoff.location_id) == str(airport["id"])
                or str(dropoff.location_id).upper() == airport["code"]
            ):
                return airport
