...
class RoleTemplate(Base):
    ...
    default_access_scope: Mapped[AccessScope] = mapped_column(
        SQLEnum(AccessScope, name="access_scope"), default=AccessScope.SELF
    )
    ...
    organization: Mapped["Organization"] = relationship(
        "Organization", back_populates="role_templates"
    )
    assignments: Mapped[list["EmployeeRoleAssignment"]] = relationship(
        "EmployeeRoleAssignment", back_populates="role_template"
    )
...
class EmployeeRoleAssignment(Base):
    ...
    access_scope: Mapped[AccessScope] = mapped_column(
        SQLEnum(AccessScope, name="access_scope", create_type=False), nullable=False
    )

    # For 'individuals' scope: list of specific employee IDs
    accessible_employee_ids: Mapped[list[int] | None] = mapped_column(ARRAY(Integer), nullable=True)

    # For 'group' scope: list of department/group names
    accessible_groups: Mapped[list[str] | None] = mapped_column(ARRAY(String), nullable=True)
    ...
    role_template: Mapped["RoleTemplate"] = relationship(
        "RoleTemplate", back_populates="assignments"
    )
